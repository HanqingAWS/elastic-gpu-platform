...
def configured_types() -> list[str]:
    """机型优先级(顺序即优先级),来自平台配置;空/异常回落默认。"""
    try:
        t = get_dynamo().get_config().get("instance_type_priority") or []
...
def provision_region(region: str, ami_id: str, *, vpc_id: str | None = None,
    ...
    # 3.5) 机型:读平台配置的优先级(顺序即优先级,默认 p4d > p4de),再按该区实际供给过滤容错。
    want_types = configured_types()
